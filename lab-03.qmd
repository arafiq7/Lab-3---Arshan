...
2.  Make sure you install all the packages listed above.32
...
2.  Now as a collaborators you can collaborate with your teammates repository, by cloning their repository. Go to their repository and copy the HTTPS url to clone their repository, this will looks like this:
    ...
    ```         
    https://github.com/arafiq7/Lab-3---Arshan.git
    ```
...
Type line below here: #line 5: works well

#Jaden here. This works!
...
data(cps)
print(head(cps)) # display first rows

beeswarm(cps$hrslastwk, main = "Beeswarm of Hours Worked Last Week", ylab = "Hours")

beeswarm(hrslastwk ~ gender, data = cps,
         main = "Beeswarm of Hours Worked by Gender",
         xlab = "Gender", ylab = "Hours")
...
# Create Data
Prop <- table(cps$gender)
 
pie(Prop, labels = names(Prop),    # use category names
    main = "Distribution of Gender in CPS Data")

Prop2 <- table(cps$race)
pie(Prop2, labels = names(Prop2), main = "Distribution of Race in CPS Data")
```
...
#Jaden here. The changes are visible.
